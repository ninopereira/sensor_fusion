#!/usr/bin/env python3
"""
Generate synthetic IMU and wheel-odometry sensor readings for testing the
...
Frame conventions (important - these are simplifications the current filter relies on):
  ...
  - dx, dy are WORLD-frame position deltas (true displacement + noise), matching the
    filter's H (absolute position measurement), not body-frame wheel-odometry output.
    A real wheel encoder reports body-frame displacement that still needs a heading
    estimate to rotate into world frame before hitting this filter's H; that rotation
    step doesn't exist in this repo, so it's folded into the generator instead.
...
# --- sensor parameters: must match the ImuSensor/OdometrySensor construction in main.cpp ---
IMU_FREQ_HZ = 1000.0
...
def simulate() -> tuple[
    ...
    odom_rows: list[tuple[float, float, float]] = []
    prev_x, prev_y = 0.0, 0.0
    for t_odom, cx, cy, _theta, _v in odom_true:
        dx = (cx - prev_x) + random.gauss(0.0, ODOM_POS_NOISE_STD_M)
        dy = (cy - prev_y) + random.gauss(0.0, ODOM_POS_NOISE_STD_M)
        odom_rows.append((t_odom, dx, dy))
        prev_x, prev_y = cx, cy

    return imu_rows, odom_rows, odom_true
...
def main():
    imu_rows, odom_rows, odom_true = simulate()
